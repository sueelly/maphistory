...
public class MainActivity extends AppCompatActivity {
    ...
    private final LocationListener mLocationListener = new LocationListener() {
        ...
        public void onLocationChanged(Location location) {
            ...
            sb.append("위치 정보: ").append(provider).append('\n');
            sb.append("경도: ").append(longitude).append('\n');
            sb.append("위도: ").append(latitude).append('\n');
            sb.append("고도: ").append(altitude).append('\n');
            sb.append("정확도: ").append(accuracy).append('\n');
            LlXyConverter.init();
            Point2D point =  LlXyConverter.LonLat2xy(longitude,latitude);
            sb.append("x: ").append((int)point.x).append(' ');
            sb.append("y: ").append((int)point.y).append('\n');

            tv.setText(sb.toString());

            try {
                writeFile("x_and_y.txt", (int) point.x + " "+(int) point.y + "\n");
                String rFile = readFile("x_and_y.txt");
                tv.setText(rFile);
            } catch (IOException e) {
                e.printStackTrace();
            }
